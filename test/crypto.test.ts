...
// RFC 7748, section 6.1 — the official X25519 (Diffie-Hellman) test vector
...
test('X25519 matches the RFC 7748 test vector', () => {
...
test('ECDH works with a public key prefixed by 0x05', () => {
	assert.deepEqual(Curve.sharedKey(ALICE_PRIV, addKeyType(BOB_PUB)), SHARED)
})

test('generateKeyPair produces a coherent pair', () => {
...
test('stripKeyType and addKeyType are inverses', () => {
...
test('HKDF matches the RFC 5869 test vector', () => {
...
test('known SHA-256 and HMAC-SHA256 values', () => {
...
test('AES-256-GCM round-trips and rejects a wrong AAD', () => {
	const key = Buffer.alloc(32, 7)
	const iv = Buffer.alloc(12, 3)
	const aad = Buffer.from('header')
	const msg = Buffer.from('hello jester')

	const enc = aesEncryptGCM(msg, key, iv, aad)
	assert.deepEqual(aesDecryptGCM(enc, key, iv, aad), msg)
	assert.throws(() => aesDecryptGCM(enc, key, iv, Buffer.from('other')))
})
